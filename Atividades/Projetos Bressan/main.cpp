...
Descrição: este projeto é um sistema em C++ para cadastro, consulta, exclusão e geração de relatórios de imóveis residenciais e comerciais. Os dados são armazenados em um arquivo texto ("BD_Imoveis2.txt") e o sistema permite diversas operações sobre a base de imóveis.
*/
...
//Limite máximo que pode conter dentro do vetor
...
int lerImoveis(Imovel imoveis[], int max)
{
    ...
    string cabecalho;
    getline(arquivo, cabecalho); // Ignora o cabeçalho, se houver
...
int main()
{
    ...
    do
    {
        ...
        {
            // Sair do programa.
            salvarImoveis(imoveis, nImoveis);// Salva os imóveis antes de sair
        ...
        case 3:
        {
            ...
            for (int i = 0; i < nImoveis; i++)
            {
                if (imoveis[i].valor >= valorMin && imoveis[i].valor <= valorMax)
                {
                    ...
                    // ...adicione outros campos se quiser mostrar mais informações...
                }
            }
            if (!encontrou)
            {
